...
export default function ApiIntegrationPage() {
  ...
  const loadItems = async () => {
    ...
    const result = await apiCall<Item[]>(fastApiConfig.endpoints.items.list.replace(/\/$/, ''))
    if (result.success) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Web App - FastAPI 服务集成</h1>
          <p className="mt-2 text-gray-600">
            测试和调用 FastAPI Web 服务的所有 API 接口 (Web 应用版本)
          </p>
